...
void renderSkyObstacle(Obstacle* obstacle)
{
    ...
    glVertex3f(obstacleX - 50.0f, obstacleY - rocketOffset, 0.0f);        // Top left (rocket length is halved from 100 to 50)
    glVertex3f(obstacleX - 50.0f, obstacleY + 15.0f - rocketOffset, 0.0f);  // Bottom left
    glVertex3f(obstacleX, obstacleY + 15.0f - rocketOffset, 0.0f);        // Bottom right
    glEnd();

    // Rocket tip (triangle) - adjusted to match the new body width
    glBegin(GL_TRIANGLES);
    glColor3f(1.0f, 0.0f, 0.0f);  // Red color for the tip
    glVertex3f(obstacleX - 50.0f, obstacleY - 5.0f - rocketOffset, 0.0f);   // Bottom left point
    glVertex3f(obstacleX - 70.0f, obstacleY + 7.5f - rocketOffset, 0.0f);   // Left point (aligned with the new body width)
    glVertex3f(obstacleX - 50.0f, obstacleY + 20.0f - rocketOffset, 0.0f);  // Top right point
    glEnd();
    ...
    // Top fin
    glVertex3f(obstacleX, obstacleY + 15.0f - rocketOffset, 0.0f);         // Top point of the rocket body
    glVertex3f(obstacleX, obstacleY + 25.0f - rocketOffset, 0.0f);         // Farther out to the right
    glVertex3f(obstacleX - 10.0f, obstacleY + 15.0f - rocketOffset, 0.0f); // Attached to the body (adjusted for the narrower body)

    // Bottom fin
    glVertex3f(obstacleX, obstacleY - rocketOffset, 0.0f);                 // Bottom point of the rocket body
    glVertex3f(obstacleX, obstacleY - 10.0f - rocketOffset, 0.0f);         // Farther out to the right
    glVertex3f(obstacleX - 10.0f, obstacleY - rocketOffset, 0.0f);         // Attached to the body (adjusted for the narrower body)
    glEnd();
}
